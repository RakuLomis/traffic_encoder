...
if __name__ == '__main__':
    ...
    if USE_FLOW_FEATURES_THIS_RUN:
        print("\n[2.5/4] Performing Feature Engineering 4.0 (Gold Standard Diagnostic)...")

        # a) 定义 *新* 的流特征名称 (不变)
        flow_feature_names = [
            'flow_avg_len', 'flow_std_len', 'flow_pkt_count',
            'flow_avg_iat', 'flow_std_iat', 'flow_max_iat',
            'flow_duration_per_pkt',  # <-- 【修复】
            'flow_large_pkt_ratio'    # (这个是正确的)
        ]
        
        # b) 【!! 核心修改：Pass 0 - 从 *源* 文件学习 !!】
        print(f" -> [Pass 0] Loading *required columns* from source file: {SOURCE_CSV_PATH}")
        # (只加载我们计算统计数据所需的列，以节省内存)
        try:
            use_cols = ['tcp.stream', 'ip.len', 'tcp.options.timestamp']
            stats_df = pd.read_csv(SOURCE_CSV_PATH, usecols=use_cols, dtype=str)
        except (FileNotFoundError, ValueError) as e:
            print(f"!!! 致命错误: 无法加载源CSV文件 '{SOURCE_CSV_PATH}'.")
            print("    请在 __main__ 中正确设置 SOURCE_CSV_PATH 变量。")
            print(f"    错误: {e}")
            exit()
            
        print(f" -> Source file loaded ({len(stats_df)} rows). Converting hex/time columns...")
        # c) 【!! 核心修改：在 *源* DF 上转换 !!】
        stats_df['ip.len_temp_dec'] = stats_df['ip.len'].apply(robust_hex_to_int).astype(np.float32)
        stats_df['tcp.stream_temp_dec'] = stats_df['tcp.stream'].apply(robust_hex_to_int).astype(np.int32)
        stats_df['time_temp_dec'] = stats_df['tcp.options.timestamp'].apply(robust_timestamp_to_tsval).astype(np.int64)
        ...
        # d) 【!! 核心修改：计算 *真正* 的全局统计数据 !!】
        print(" -> Calculating *TRUE* global flow stats...")
        # (我们不需要 global_defaults，所以 is_train_set=False)
        global_true_maps = calculate_flow_stats(stats_df, is_train_set=False)
        ...
        for df in [train_df, val_df_aligned, test_df_aligned]:
            ...
            # (ip.len_temp_dec 和 time_temp_dec 不在这些df上，无需清理)
        
        # h) 【最终清理】(不变, 使用 np.nan_to_num)
        ...
        print(f" -> Feature Engineering 4.0 complete. {len(flow_feature_names)} flow features created.")
