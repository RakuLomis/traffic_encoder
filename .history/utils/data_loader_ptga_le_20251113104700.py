...
class GNNTrafficDataset(Dataset):
    ...
    def _preprocess_all(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 【!! 核心修复：用 NumPy 替换 Pandas !!】
        print("\nConverting processed DataFrame to NumPy dictionary for fast worker access...")
        ...
        for col in tqdm(df.columns, desc="Converting columns to NumPy"):
            # .values 会将其转换为 NumPy 数组
            # .copy() 确保它在 del df 后仍然存在
            numpy_data_dict[col] = df[col].values.copy() 
        
        # (释放 DataFrame 内存)
        del df
        gc.collect() # (确保 import gc)
        ...
        for expert_name, graph_info in self.expert_graphs.items():
            ...
            for field_name in all_nodes_list:
                ...
                if is_real_node:
                    ...
                    # 【!! 核心修复：使用快速的 NumPy 索引 !!】
                    if field_name in self.processed_df:
                        # self.processed_df 是 NumPy 字典
                        # [field_name] 获取列 (np.ndarray)
                        # [idx] 获取该行的值
                        value = self.processed_df[field_name][idx]
                    else:
                        value = None
