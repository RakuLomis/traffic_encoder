...
class GNNTrafficDataset(Dataset):
    """
    【!! MoE 最终版 V2：单一 Data 对象 + Tensor Cache !!】
    
    为 HierarchicalMoE (多视角)模型准备数据。
    - __init__: 构建 *每个专家* 的图骨架 (self.expert_graphs)
    - __init__: 预处理并创建 "Tensor Cache"
    - __getitem__: 返回一个 *单一* 的 Data 对象，该对象将
                   专家子图(eth, ip)和流特征(flow_stats)作为 *属性* 携带。
    """
    def __init__(self, dataframe: pd.DataFrame, config_path: str, vocab_path: str, 
                 use_flow_features: bool = False):
        super().__init__()
        print(f"\nInitializing MoE GNNTrafficDataset (Single Data Output, Flow: {use_flow_features})...")
        ...
        self.flow_feature_names = [ # (确保这与你的特征工程匹配)
        ...
        # --- 注入端口分箱逻辑 (YAML type='port_binned') ---
        self.port_fields = {'tcp.srcport', 'tcp.dstport'}
        
        # --- 定义十进制字段 (YAML type='numerical') ---
        ...
        for name, expert_fields in self.expert_definitions.items():
            ...
            self.all_real_gnn_nodes.update(real_nodes_for_expert)
        ...
        # (将 GNN 节点也添加到总处理列表)
        self.all_feature_cols_to_process.update(self.all_real_gnn_nodes)
    ...
    def _preprocess_all(self, df: pd.DataFrame, cols_to_process: set) -> pd.DataFrame:
        ...
        # --- (辅助函数: robust_hex_to_int, robust_timestamp_to_tsval, bin_port_from_hex) ---
        ...
        for field_name in tqdm(cols_to_process, desc="Pre-processing columns"):
            if field_name not in cols_in_df: continue
    ...
    def __getitem__(self, idx) -> Data: # <-- 【!! 修改：返回一个 Data !!】
        """
        【MoE 版 - 单一 Data 对象】
        返回一个 *单一* 的 Data 对象，该对象将
        专家子图(eth, ip)和流特征(flow_stats)作为 *属性* 携带。
        """
        ...
        for expert_name, graph_info in self.expert_graphs.items():
            ...
            # 4. 【关键】将子图 *作为属性* 附加到容器上
            setattr(container_data, expert_name, sub_data)

        # 5. (可选) 附加流特征
        if self.use_flow_features:
            flow_stats_list = [self.tensor_cache[f][idx] for f in self.flow_feature_names]
            container_data.flow_stats = torch.stack(flow_stats_list).view(1, -1) 
