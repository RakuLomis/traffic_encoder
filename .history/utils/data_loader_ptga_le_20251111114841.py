...
class GNNTrafficDataset(Dataset):
    ...
    def __init__(self, dataframe: pd.DataFrame, config_path: str, vocab_path: str, 
        ...
        print(f"Pre-processing all {len(self.all_real_gnn_nodes | set(self.flow_feature_names))} columns...")
        # processed_df = self._preprocess_all(dataframe, self.all_real_gnn_nodes | self.flow_feature_names)
        processed_df = self._preprocess_all(dataframe, self.all_real_gnn_nodes | set(self.flow_feature_names))
        ...
        for col_name in tqdm(self.all_real_gnn_nodes | set(self.flow_feature_names), desc="Creating tensor cache"):
