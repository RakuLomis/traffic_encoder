...
if __name__ == '__main__':
    ...
    if USE_FLOW_FEATURES_THIS_RUN:
        print("\n[2.5/4] Performing Feature Engineering 2.0 (IAT, Duration, Bins)...")
        ...
        # c) 【!! 核心修改：定义一个 *函数* 来计算所有统计数据 !!】
        #    (这避免了重复代码，并修复了 IAT bug)
        def calculate_flow_stats(df, is_train_set=False):
            """
            在 *单个* DataFrame 上计算所有8个流特征。
            ...
            maps['flow_duration'] = (flow_max_time - flow_min_time)
            
            LARGE_PKT_THRESHOLD = 1400 
            large_pkts = df[df['ip.len_temp_dec'] > LARGE_PKT_THRESHOLD]
            maps['flow_large_pkt_count'] = large_pkts.groupby('tcp.stream_temp_dec').size()
            
            if is_train_set:
                # --- (计算全局默认值) ---
                print("     -> Calculating global defaults...")
                defaults = {
                    'flow_avg_len': float(maps['flow_avg_len'].mean()),
                    'flow_std_len': float(maps['flow_std_len'].mean()),
                    'flow_pkt_count': int(maps['flow_pkt_count'].mean()),
                    'flow_avg_iat': float(maps['flow_avg_iat'].mean()),
                    'flow_std_iat': float(maps['flow_std_iat'].mean()),
                    'flow_max_iat': float(maps['flow_max_iat'].mean()),
                    'flow_duration': float(maps['flow_duration'].mean()),
                    'flow_large_pkt_count': 0 # Count 默认为 0
                }
                # 清理 defaults 中的 nan/inf
                for k, v in defaults.items():
                    if np.isnan(v) or np.isinf(v): defaults[k] = 0
        ...
        # 【!! 在这里设置你的实验 !!】
        # OPEN_WORLD = True   # 现实世界（使用全局默认值）
        OPEN_WORLD = False  # 诊断实验（使用“泄露”的统计）
        ...
        if OPEN_WORLD:
            ...
            for df_name, df in [('Validation', val_df_aligned), ('Test', test_df_aligned)]:
                ...
                # b. 【!! 修复 !!】将【自身】的统计数据map回自身
                for f_name in flow_feature_names:
                    # 使用 *它自己* 的 map，并用 0 填充（而不是全局默认值）
                    df[f_name] = df['tcp.stream_temp_dec'].map(df_maps[f_name]).fillna(0)
        ...
        for df in [train_df, val_df_aligned, test_df_aligned]:
             ...
             for col in flow_feature_names: 
                 ...
                 if col not in df.columns:
                     raise KeyError(f"在最终清理时, 列 '{col}' 未在 {df_name} 中找到！逻辑错误！")
        ...
        print(f" -> Feature Engineering 2.0 complete.")
