...
if __name__ == '__main__':
    ...
    MAX_PACKETS_PER_FLOW_TRAIN = 128
    # MAX_PACKETS_PER_FLOW_EVAL = 256
    MAX_PACKETS_PER_FLOW_EVAL = MAX_PACKETS_PER_FLOW_TRAIN
    PACKET_CAP_STRATEGY = 'flow_uniform'  # 'flow_head' | 'flow_uniform' | 'flow_random'
    ...
    FOCAL_GAMMA = 2.0   
    LOSS_TYPE = 'weighted_ce'  # 'focal' | 'weighted_ce'
    EPSILON = 1e-6 
    ...
    print(f"Loss type: {LOSS_TYPE} (focal_gamma={FOCAL_GAMMA})")
    ...
    def cap_packets_per_flow(df: pd.DataFrame, split_name: str, cap_per_flow: int = None) -> pd.DataFrame:
        ...
        if PACKET_CAP_STRATEGY in ('flow_head', 'flow_uniform'):
            ...
            if time_col is not None:
                ...
                if PACKET_CAP_STRATEGY == 'flow_head':
                    ...
                    selected_index = []
                    for _, g in work.groupby('stream_id', sort=False):
                        m = len(g)
                        if m <= cap_per_flow:
                            selected_index.extend(g.index.tolist())
                        else:
                            # Uniformly cover start/middle/end packets in one flow.
                            pos = np.linspace(0, m - 1, num=cap_per_flow)
                            pos = np.clip(np.round(pos).astype(np.int64), 0, m - 1)
                            pos = np.unique(pos)
                            if len(pos) < cap_per_flow:
                                all_pos = np.arange(m, dtype=np.int64)
                                mask = np.ones(m, dtype=bool)
                                mask[pos] = False
                                fill = all_pos[mask][: (cap_per_flow - len(pos))]
                                pos = np.sort(np.concatenate([pos, fill]))
                            selected_index.extend(g.iloc[pos].index.tolist())
                    capped = work.loc[selected_index].drop(
                        columns=['_time_order_num', '_orig_idx'],
                        errors='ignore',
                    )
                    print(
                        f"[Packet Cap] {split_name}: strategy=flow_uniform, "
                        f"time_col='{time_col}', cap={cap_per_flow}"
                    )
            else:
                ...
                print(
                    f"[Packet Cap] {split_name}: strategy={PACKET_CAP_STRATEGY} fallback to flow_random "
    ...
    if TRAIN_TARGET == 'flow' and 'stream_id' in train_df.columns:
        # Use flow-level class distribution for focal alpha when optimizing flow loss.
        ...
        # Fallback to packet-level class distribution.
    ...
    print(f" -> Class-weight count level: {alpha_count_level}")
    print(f" -> Class counts: {class_counts_np.tolist()}")
    print(f" -> Class weights: {alpha_weights.cpu().numpy().round(2)}")
