...
class FlowReprLogitAggregator(nn.Module):
    ...
    def forward_grouped(
        ...
        r = self.repr_proj(packet_repr.to(dtype=target_dtype))
        l = self.logit_proj(packet_logits.to(dtype=target_dtype))
        h = self.fuse_mlp(torch.cat([r, l], dim=-1))  # [N, H]

        scores = self.attn_head(h).squeeze(-1)  # [N]
        ...
        alpha = exp_scores / (denom.index_select(0, inverse_flow_index) + 1e-12)  # [N]

        flow_repr = torch.zeros((num_flows, h.size(1)), dtype=h.dtype, device=h.device)
        flow_repr.index_add_(0, inverse_flow_index, h * alpha.unsqueeze(-1))
        return self.flow_head(flow_repr)
