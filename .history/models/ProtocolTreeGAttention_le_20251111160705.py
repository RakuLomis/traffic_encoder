...
class HierarchicalMoE(nn.Module):
    ...
    #     gnn_gated = gnn_normed * gate_weights[0]
    #     final_embeddings_list = [gnn_gated]
    ...
    #         # 【!!】应用流专家的门控 (gate_weights[1])
    #         flow_gated = flow_normed * gate_weights[1] 
    #         final_embeddings_list.append(flow_gated)
    ...
    def forward(self, batch: Batch) -> torch.Tensor: # <-- 【修改】
        """
        【重构版 - 接收单一 Batch 对象】
        """
        gnn_embeddings = []
        ...
        for expert_name, expert_model in self.experts.items():
            ...
            # 【修改】从批处理对象中，按属性获取这位专家的数据
            if not hasattr(batch, expert_name):
                continue 
                
            data_for_this_expert = getattr(batch, expert_name)
            # (数据已经在 device 上了)
        ...
        if self.use_flow_features:
            # 【修改】从 batch 属性获取
            if not hasattr(batch, 'flow_stats'):
                 raise ValueError("...")
            
            flow_stats_input = batch.flow_stats # 应该是 [B, num_features]
