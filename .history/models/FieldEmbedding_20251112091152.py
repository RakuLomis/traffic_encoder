    ...
    def __init__(self, config_path, vocab_path):
        ...
        for field_name in tqdm(self.field_to_key_map, desc="Initing embedding layer and constructing tensor maps... "):
            ...
            if field_type == 'categorical':
                ...
                embedding_dim = field_config['embedding_dim']
                # layer = nn.Sequential(
                #     nn.LayerNorm(1), # 归一化单个输入 (e.g., 443.0)
    ...
    def forward(self, batch_data_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for field_name, input_tensor in batch_data_dict.items():
            ...
            if layer_key and layer_key in self.embedding_layers:
                ...
                # # 【!! 核心修改：确保 numerical 输入是 [B, 1] 的 float !!】
                # elif isinstance(layer, nn.Sequential) and isinstance(layer[0], nn.LayerNorm):
                # # (这是我们新的 numerical 块)
                #     if processed_input.dim() == 1:
                #         processed_input = processed_input.unsqueeze(1) # [B] -> [B, 1]
                #     processed_input = processed_input.float() 

                # 【!! 核心修改：确保 numerical 输入是 [B, 1] 的 float !!】
                elif isinstance(layer, nn.Sequential) and isinstance(layer[0], nn.LayerNorm):
                # (这是我们新的 numerical 块)
                    # [!! 修复 !!] 我们应该操作 `input_tensor`
                    if input_tensor.dim() == 1:
                        input_tensor = input_tensor.unsqueeze(1) # [B] -> [B, 1]
