...
def custom_collate_flat_dict(batch_list: List[Dict[str, torch.Tensor]]) -> Dict[str, Any]:
    """
    自定义 collate 函数，用于 GNNTrafficDataset (扁平字典版)。
    它将 {'eth_edge_index': ...} 转换为 {'eth': <Batch object>, ...}
    """
    # 1. 分离图数据和简单张量
    simple_tensors_batch = defaultdict(list)
    graph_data_by_expert = defaultdict(list)
    expert_prefixes = set() 

    # 从第一个样本中动态识别所有专家前缀
    if batch_list:
        for key in batch_list[0].keys():
            if key.endswith('_edge_index'):
                expert_prefixes.add(key.split('_edge_index')[0] + '_') # 例如 'eth_'

    if not expert_prefixes:
        # 如果没有图，就退回为默认 collate
        return default_collate(batch_list)

    for item_dict in batch_list:
        graph_item_parts = defaultdict(dict)
        for key, value in item_dict.items():
            ...
            for prefix in expert_prefixes:
                if key.startswith(prefix):
                    # 这是一个图特征
                    # 移除前缀，恢复原始字段名
                    original_field_key = key[len(prefix):].replace('__', '.')
                    
                    if key.endswith('_edge_index'):
                         graph_item_parts[prefix]['edge_index'] = value
                    elif key.endswith('_num_nodes'):
                         graph_item_parts[prefix]['num_nodes'] = value
                    else:
                         graph_item_parts[prefix][original_field_key] = value
                    is_graph_part = True
                    break # 键只属于一个专家
            
            if not is_graph_part:
                # 这是一个简单张量 (例如 'y', 'flow_stats')
                simple_tensors_batch[key].append(value)

        # 为这个 item 创建所有 Data 对象
        for prefix, features in graph_item_parts.items():
            expert_name = prefix[:-1] # 移除 '_'
            num_nodes = features.pop('num_nodes', torch.tensor([1])).item() # 提取 num_nodes
            
            # (确保 edge_index 存在，否则 PyG 会报错)
            if 'edge_index' not in features:
                features['edge_index'] = torch.empty((2, 0), dtype=torch.long)
                
            graph_data = Data(num_nodes=num_nodes, **features)
            graph_data_by_expert[expert_name].append(graph_data)

    # 2. 使用 default_collate 批处理简单张量
    collated_batch = {key: default_collate(values) for key, values in simple_tensors_batch.items()}

    # 3. 使用 Batch.from_data_list 批处理每个专家的图数据
    for expert_name, data_list in graph_data_by_expert.items():
        collated_batch[expert_name] = Batch.from_data_list(data_list)
