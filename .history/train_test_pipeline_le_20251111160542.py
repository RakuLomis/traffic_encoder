...
def train_one_epoch(
    ...
    for i, batch in enumerate(tqdm(dataloader, desc="Training")): # <-- 【修改】
        ...
        outputs, gates_dict = model(batch) # <-- 【修改】
...
def evaluate(
    ...
    for i, batch in enumerate(tqdm(dataloader, desc="Evaluating")): # <-- 【修改】
        # ==================== 核心修改点：分别移动值 ====================
        #
        # 1. 【新】遍历字典，将【每一个值】（图和张量）分别移动到GPU
        #
