...
if __name__ == '__main__':
    ...
    if USE_FLOW_FEATURES_THIS_RUN: 
        ...
        # b) 【!! 关键修复：在 groupby 之前转换 Hex !!】
        print(" -> [Hex Fix] Converting 'ip.len' from Hex to Int for all sets...")
        for df_name, df in [('Train', train_df), ('Validation', val_df_aligned), ('Test', test_df_aligned)]:
            if df.empty: continue
            print(f"   -> Processing {df_name} set for Hex conversion...")
            # 1. 转换 ip.len
            #    使用 .apply() 来处理十六进制字符串
            df['ip.len'] = df['ip.len'].apply(robust_hex_to_int).astype(np.float32)
            # 2. 转换 tcp.stream
            #    假设 tcp.stream 是十进制，但我们用 apply 以防万一
            df['tcp.stream'] = df['tcp.stream'].apply(robust_hex_to_int).astype(np.int32)
        print(" -> [Hex Fix] All 'ip.len' and 'tcp.stream' columns are now decimal integers.")
