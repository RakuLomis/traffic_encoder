    ...
    def forward(self, batch_data_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for field_name, input_tensor in batch_data_dict.items():
            ...
            if layer_key and layer_key in self.embedding_layers:
                ...
                # 检查这是否是我们的“数值型”Sequential块 (BatchNorm -> Linear)
                elif isinstance(layer, nn.Sequential) and isinstance(layer[0], nn.BatchNorm1d):
                    # 
                    # 1. 确保输入是 2D: [B] -> [B, 1]
                    if input_tensor.dim() == 1:
                        input_tensor = input_tensor.unsqueeze(1) 
                    # 2. 确保类型是 float (这一步在 GNNTrafficDataset 中已完成，但双重保险无害)
