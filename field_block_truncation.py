...
test_directory_path = os.path.join(current_path, 'Data', 'Test') 
input_directory_path = os.path.join('..', 'TrafficData', 'dataset_29_d1_csv_merged') 
...
if csv_path_list is not None: 
    for csv_path, csv_name in zip(csv_path_list, csv_name_list): 
        ...
        for block_num in range(len(list_df_block)): 
            output_csv_in_fold(list_df_block[block_num], os.path.join(input_directory_path, csv_name, DISCRETE_BLOCK), f'{block_num}' + '.csv') 
