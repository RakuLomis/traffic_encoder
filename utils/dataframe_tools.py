...
def stratified_hybrid_sample_dataframe_optimized(
    df: pd.DataFrame, 
    label_column: str, 
    proportion: float, 
    random_state: Optional[int] = 42
    ...
    """
    【内存优化版】对 DataFrame 进行按比例分层抽样，并为少数类设置“下限”。

    此版本通过直接操作索引 (index) 来避免 `groupby().apply()` 
    带来的巨大内存开销，适用于数百万行级别的巨大数据集。

    (逻辑与原版完全相同)
    """
    
    print(f"开始混合分层抽样 (Memory-Optimized)，目标比例: {proportion * 100:.1f}%...")
    ...
    print(f"按 '{label_column}' 列进行分组 (索引模式)...")
    
    # 【优化点 2】: .groupby().groups 只分组索引，不复制数据，内存占用极低
    # 它会创建一个字典: {'class_A': [idx1, idx2...], 'class_B': [idx3, idx4...]}
    index_groups = df.groupby(label_column).groups
    
    final_indices = []
    
    # 【优化点 3】: 使用 Numpy 的随机生成器，更高效
    rng = np.random.default_rng(random_state)
    
    print("应用混合抽样逻辑 (索引模式)...")
    for label, N_A in tqdm(class_counts.items(), desc="Sampling classes"):
        
        # --- 1. 计算 n_sample (与你原函数的逻辑 *完全一致*) ---
        n_sample = 0
        if N_A <= n_avg:
            # 【规则4】
            n_sample = N_A
        ...
        # --- 2. 抽样索引 (而不是 DataFrame) ---
        
        # 从字典中获取这个类的所有索引
        class_indices = index_groups[label]
        
        # 使用 numpy.random.choice 高效抽样索引
        # replace=False 因为你的逻辑中 n_sample 总是 <= N_A
        ...
        # --- 3. 收集抽样后的索引 ---
        final_indices.append(sampled_group_indices)

    # 【优化点 4】: 将所有索引列表拼接成一个大的一维 Numpy 数组
    print("合并所有抽样索引...")
    final_indices = np.concatenate(final_indices)

    # 【优化点 5】: 最后，执行 *一次* .loc[] 操作
    # 这是唯一一次内存复制，只复制我们需要的数据
    print("从原始 DataFrame 中提取最终样本...")
    sampled_df = df.loc[final_indices]
...
def stratified_hybrid_sample_from_csv_stream(
    csv_path: str,
    label_column: str,
    proportion: float,
    ...
    """
    在不一次性载入全部数据的前提下，对一个很大的 CSV 文件做
    「混合分层抽样」（逻辑与你原来的函数一致），
    内存占用约等于『抽样后 DataFrame 的大小』。
    ...
    proportion : float
        抽样比例 (0, 1]
    chunksize : int
        每个 chunk 的行数，视内存大小调节
    random_state : int
        随机种子
    read_csv_kwargs : dict
        透传给 pd.read_csv 的其他参数（如 usecols, dtype 等）
    """
    ...
    if proportion == 1.0:
        # 注意：这个场景本身就意味着要保留全部数据，
        # 内存占用必然接近原始数据集大小，没办法“省掉”。
        print("proportion=1.0：将流式读取整个 CSV 并返回完整 DataFrame。")
        df_full = pd.read_csv(csv_path, **read_csv_kwargs)
        df_full = df_full.sample(frac=1, random_state=random_state).reset_index(drop=True)
        return df_full

    print(f"开始流式混合分层抽样，目标比例: {proportion * 100:.1f}%...")
    rng = np.random.default_rng(random_state)
    ...
    columns_ref = None  # 记录列顺序

    for chunk in tqdm(pd.read_csv(csv_path, chunksize=chunksize, **read_csv_kwargs),desc="Handling chunks. "):
        if columns_ref is None:
            columns_ref = list(chunk.columns)

        vc = chunk[label_column].value_counts()
        for label, cnt in vc.items():
            class_counts[label] += int(cnt)
    ...
    n_avg = int(round((proportion * n_total) / n_classes))
    print(f" -> 总样本: {n_total}, 类别数: {n_classes}")
    print(f" -> 平均抽样数 (n_avg) = {n_avg}")

    # 计算每个类别需要抽多少
    n_sample_per_class: Dict = {}
    for label, N_A in tqdm(class_counts.items(), desc="Calculating sample number. "):
        if N_A <= n_avg:
            n_sample = N_A
        else:
            # n_A_prop = int(round(N_A * proportion))
            ...
            # else:
            #     n_sample = n_A_prop
        n_sample_per_class[label] = int(n_sample)

    print("各类别目标抽样数 (部分展示):")
    for i, (lbl, n_s) in enumerate(n_sample_per_class.items()):
        print(f"  - 类别 {lbl}: {n_s} 条")
        if i >= 10:  # 只打印前几个
            print("  ...")
            break

    # =========================
    # 第二遍：按类 reservoir sampling
    # =========================
    print("第二遍：执行按类 reservoir sampling (流式)...")

    # 每类已经看到多少条
    seen_per_class: Dict = defaultdict(int)
    # 每类的 reservoir，存 Series；最终再合并成 DataFrame
    ...
    for chunk in tqdm(pd.read_csv(csv_path, chunksize=chunksize, **read_csv_kwargs), desc="Sampling chunks. "):
        ...
        for _, row in chunk.iterrows():
            ...
            # 若该类不需要采样（n_sample=0），直接跳过
            target_n = n_sample_per_class.get(label, 0)
            if target_n == 0:
                continue

            seen_per_class[label] += 1
            k = seen_per_class[label]  # 当前是该类的第 k 条样本

            if k <= target_n:
                # reservoir 还没填满，直接放入
                reservoir_per_class[label].append(row)
            else:
                # reservoir sampling：以 target_n/k 的概率替换一个已有样本
                j = rng.integers(1, k + 1)  # 均匀采样 [1, k]
                if j <= target_n:
                    # 替换第 j-1 个位置
                    reservoir_per_class[label][j - 1] = row
    ...
    if not sampled_rows:
        print("警告：没有任何样本被抽中，返回空 DataFrame。")
        return pd.DataFrame(columns=columns_ref if columns_ref is not None else [])

    sampled_df = pd.DataFrame(sampled_rows)
    if columns_ref is not None:
        sampled_df = sampled_df.reindex(columns=columns_ref)

    print(f"抽样完成：原始约 {n_total} 行 -> 抽样后 {len(sampled_df)} 行。")
